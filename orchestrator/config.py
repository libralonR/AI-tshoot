...
class Config:
    ...
    def _load_traces_catalog(self) -> list:
        """Carrega o catálogo de queries TraceQL do steering file traces-catalog.md.

        Mesmo formato do metrics-catalog: blocos YAML em fences ```yaml dentro
        do markdown. Cada entry deve ter `name`, `kind` (search | metrics_instant |
        metrics_range) e `query_template`. Para `metrics_range` é recomendado
        definir `step`. Para `search` é recomendado definir `limit`.
        """
        import re
        import yaml
        ...
        for idx, block in enumerate(yaml_blocks, 1):
            try:
                ...
                if isinstance(parsed, list):
                    ...
                    log.warning(f"[Config] traces YAML block {idx}: not a list, got {type(parsed).__name__}")
            except Exception as e:
        ...
        log.info(
            f"[Config] Loaded traces catalog | "
            f"total_entries={len(valid)} | "
            f"categories={list(set(e.get('category', '?') for e in valid))} | "
