"""Geração e ranking de hipóteses.

Mesma implementação da versão atual em `orchestrator/hypothesis.py`,
apenas com imports do novo pacote de domínio.
"""
...
class HypothesisGenerator:
    ...
    def _extract_specific_cause(self, evidences: List[Evidence]) -> Optional[str]:
        """Extrai causa raiz específica a partir do conteúdo dos alertas."""
        for e in evidences:
    ...
    def _generate_next_steps(self, component: str, evidences: List[Evidence]) -> List[NextStep]:
        ...
        steps.extend(self._golden_signal_steps(component))
        steps.extend(self._infrastructure_steps(component))
    ...
    def _golden_signal_steps(self, component: str) -> List[NextStep]:
        ...
        saturation = [
            e for e in self.metrics_catalog
            if e.get("category") == "golden_signal"
            and e.get("name", "") in ("cpu_usage", "memory_usage", "memory_usage_percent")
        ]
        ...
        if errors:
            ...
            steps.append(NextStep(
                ...
                query=query, readOnly=True, priority=Priority.HIGH,
            ))
        ...
        if traffic:
            ...
            steps.append(NextStep(
                ...
                query=query, readOnly=True, priority=Priority.MEDIUM,
            ))
